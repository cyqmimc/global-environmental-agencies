...
1. Initial load fetches `countries-core.json` + `wb-latest.json` + `sdg-latest.json` in parallel, merging by `isoCode` into `country.wb` and `country.sdg`. The SDG file is a build-time static snapshot of four official UN environmental indicators; the browser never calls the UN API.
...
**WorldMap SVG coloring:** `src/WorldMap.jsx` injects `fill` colors into the raw SVG via regex matching `<path id="xx">` or `<g id="xx">` where `xx` is a 2-letter ISO code. Nine switchable indicators are defined in `METRIC_CONFIG`. **Critical:** if compressing `world-map.svg` with SVGO, you must disable `cleanupIds` or the map will render all black.
